...
#3) Lithiated Nickel Cobalt Aluminum Oxide :
# DOD = 40%
DOD3 = 40/100
# Energy yield/density = 80 W-hr/kg
Ed3 = 153
...
while True:
    ...
    elif x == 3 :   #This example for NCA
        #This example for NCA
        print("-----This example for NCA-----")
        Pe = float(input("Orbit average power, Enter Pe value = "))    # Pe = 110 W
        Te = 33.85  # [min]
        DOD3 = 40/100
        n = 0.9
        Cr3 = (Pe * (Te / 60)) / (DOD3 * n)
        print("Cr = {} [W-hr]".format(Cr3))     #110 W icin 137 W baglamamız gerekiyor
        #Allowable range = 26V - 32V
        Cells3min = 26 / 3.0
        Cells3max = 32 / 4.1
        print("Min = {} [cells], Max = {} [cells]".format(Cells3min,Cells3max))
        b = float(input("Select integer in min and max range for number of cells in one series = "))
        print("{} [cells] in series".format(b))
        Cr33 = Cr3 / (b * 3.0)
        print("Cr' = {} [Amper-hr]".format(Cr33))

        if Cr33 > Cpc3 :
            print("Use 2 or more strings in parallel because capacity per cell = 43.0 Amp-hr and Cr' higher than it")
            c = 2
            d = Cpc3 * c
            ...
            if d > Cr33 :
                ...
                # At full capacity
                Cr333 = b * 3.9 * d
                print("At full capacity Cr = {} [W-hr]".format(Cr333))
                # Mass NCA battery
                Kg3 = Cr333 / Ed3
                print("Mass NCA battery = {} [kg]".format(Kg3))
            ...
            print("1 string is enough")
            print("1 strings in parallel")
            print("Capacity per cell = 43.0 Amp-hr and Cr' is not higher than it so there is not any parallel strings")
            #At full capacity
            Cr333 = b * 4.1 * Cpc3
            print("At full capacity Cr = {} [W-hr]".format(Cr333))
            #Mass NCA battery
